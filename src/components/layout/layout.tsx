...
export default function Layout({ children }: LayoutProps) {
  ...
  const handlePushImage = async () => {
    ...
    try {
      const repoName = prompt("Enter the ECR repository name:", "my-ecr-repo") || "my-ecr-repo";

      const response = await fetch(`/api/load-and-push-ecr?repoName=${encodeURIComponent(repoName)}`, {
        method: "GET",
        headers: {
      ...
      <aside className="layout-sidebar">
        ...
        <ul>
          ...
          <li>
            <>
              <button
                onClick={fetchAndDownloadDockerImage}
                style={{
